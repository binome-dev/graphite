...
async def test_siliconflow_tool_stream() -> None:
    await event_store.clear_events()
    siliconflow_tool = SiliconFlowTool.builder().is_streaming(True).api_key(api_key).build()
    content = ""
    async for messages in siliconflow_tool.invoke(
        get_invoke_context(),
...
async def test_siliconflow_tool_with_chat_param() -> None:
    chat_param = {
        "temperature": 0.1,
        "max_tokens": 15,
    }
    siliconflow_tool = SiliconFlowTool.builder().api_key(api_key).chat_params(chat_param).build()
    await event_store.clear_events()
    async for messages in siliconflow_tool.invoke(
        get_invoke_context(),
        [Message(role="user", content="Hello, my name is Grafi, how are you doing?")],
...
async def test_siliconflow_tool_with_structured_output() -> None:
...
async def test_siliconflow_tool_async() -> None:
    siliconflow_tool = SiliconFlowTool.builder().api_key(api_key).build()
    await event_store.clear_events()
...
async def test_llm_stream_node() -> None:
    ...
    llm_stream_node: Node = (
        Node.builder()
        .tool(SiliconFlowTool.builder().is_streaming(True).api_key(api_key).build())
        .build()
    )
...
async def test_siliconflow_tool_serialization() -> None:
    """Test serialization and deserialization of SiliconFlow tool."""
    await event_store.clear_events()

    # Create original tool
    original_tool = SiliconFlowTool.builder().api_key(api_key).build()
...
async def test_siliconflow_tool_with_chat_param_serialization() -> None:
    ...
    # Create original tool
    original_tool = (
        SiliconFlowTool.builder().api_key(api_key).chat_params(chat_param).build()
    )
...
async def test_siliconflow_tool_structured_output_serialization() -> None:
...
asyncio.run(test_siliconflow_tool_with_chat_param())
asyncio.run(test_siliconflow_tool_with_structured_output())
asyncio.run(test_siliconflow_tool_stream())
asyncio.run(test_siliconflow_tool_async())
asyncio.run(test_llm_stream_node())
asyncio.run(test_siliconflow_tool_serialization())
asyncio.run(test_siliconflow_tool_with_chat_param_serialization())
asyncio.run(test_siliconflow_tool_structured_output_serialization())
