...
@pytest.fixture
def siliconflow_instance():
    """Create a SiliconFlowTool instance with test configuration."""
    return SiliconFlowTool(
        system_message="dummy system message",
        name="SiliconFlowTool",
        api_key="test_api_key",
        model="Qwen/QwQ-32B",
    )


def test_init(siliconflow_instance):
    """Test that SiliconFlowTool initializes with correct attributes."""
    assert siliconflow_instance.api_key == "test_api_key"
    assert siliconflow_instance.model == "Qwen/QwQ-32B"
    assert siliconflow_instance.system_message == "dummy system message"
    assert siliconflow_instance.base_url == "https://api.siliconflow.cn/v1"


@pytest.mark.asyncio
async def test_invoke_simple_response(monkeypatch, siliconflow_instance, invoke_context):
    """Test simple text response from SiliconFlow API."""
    import grafi.tools.llms.impl.siliconflow_tool
    ...
    monkeypatch.setattr(
        grafi.tools.llms.impl.siliconflow_tool, "AsyncClient", mock_async_client_cls
    )

    input_data = [Message(role="user", content="Say hello")]
    result_messages = []
    async for message_batch in siliconflow_instance.invoke(invoke_context, input_data):
    ...
    mock_async_client_cls.assert_called_once_with(
        api_key="test_api_key", base_url="https://api.siliconflow.cn/v1"
    )


@pytest.mark.asyncio
async def test_invoke_function_call(monkeypatch, siliconflow_instance, invoke_context):
    """Test function call response from SiliconFlow API."""
    import grafi.tools.llms.impl.siliconflow_tool
    ...
    siliconflow_instance.add_function_specs(tools)
    result_messages = []
    async for message_batch in siliconflow_instance.invoke(invoke_context, input_data):
        result_messages.extend(message_batch)
...
@pytest.mark.asyncio
async def test_invoke_api_error(siliconflow_instance, invoke_context):
    """Test that API errors are properly handled and converted to LLMToolException."""
    from grafi.common.exceptions import LLMToolException

    with pytest.raises(LLMToolException, match="Error code|SiliconFlow API"):
        async for _ in siliconflow_instance.invoke(
            invoke_context, [Message(role="user", content="Hello")]
        ):
            pass


def test_to_dict(siliconflow_instance):
    """Test conversion of SiliconFlowTool instance to dictionary format."""
    result = siliconflow_instance.to_dict()
    assert result["name"] == "SiliconFlowTool"
    assert result["type"] == "SiliconFlowTool"
...
def test_prepare_api_input(siliconflow_instance):
    """Test preparation of input data for SiliconFlow API format."""
    input_data = [
    ...
    siliconflow_instance.add_function_specs(
        [
            FunctionSpec(
                name="get_weather",
                description="Get weather",
    ...
    api_messages, api_functions = siliconflow_instance.prepare_api_input(input_data)
