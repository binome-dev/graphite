...
def test_init(openkey_instance):
    assert openkey_instance.api_key == "test_api_key"
    assert openkey_instance.model == "gpt-4o-mini"
    assert openkey_instance.system_message == "dummy system message"
    assert openkey_instance.base_url == "https://openkey.cloud/v1"


@pytest.mark.asyncio
async def test_invoke_simple_response(monkeypatch, openkey_instance, invoke_context):
    import grafi.tools.llms.impl.openkey_tool
    ...
    monkeypatch.setattr(
        grafi.tools.llms.impl.openkey_tool, "AsyncClient", mock_async_client_cls
    )

    input_data = [Message(role="user", content="Say hello")]
    result_messages = []
    async for message_batch in openkey_instance.invoke(invoke_context, input_data):
    ...
    # Verify client was initialized with the right API key and base_url
    mock_async_client_cls.assert_called_once_with(
        api_key="test_api_key", base_url="https://openkey.cloud/v1"
    )


@pytest.mark.asyncio
async def test_invoke_function_call(monkeypatch, openkey_instance, invoke_context):
    import grafi.tools.llms.impl.openkey_tool
    ...
    openkey_instance.add_function_specs(tools)
    result_messages = []
    async for message_batch in openkey_instance.invoke(invoke_context, input_data):
        result_messages.extend(message_batch)
...
@pytest.mark.asyncio
async def test_invoke_api_error(openkey_instance, invoke_context):
    from grafi.common.exceptions import LLMToolException

    with pytest.raises(LLMToolException, match="Error code"):
        async for _ in openkey_instance.invoke(
            invoke_context, [Message(role="user", content="Hello")]
        ):
            pass


def test_to_dict(openkey_instance):
    result = openkey_instance.to_dict()
    assert result["name"] == "OpenKeyTool"
    assert result["type"] == "OpenKeyTool"
...
def test_prepare_api_input(openkey_instance):
    input_data = [
    ...
    openkey_instance.add_function_specs(
        [
            FunctionSpec(
                name="get_weather",
                description="Get weather",
    ...
    api_messages, api_functions = openkey_instance.prepare_api_input(input_data)
