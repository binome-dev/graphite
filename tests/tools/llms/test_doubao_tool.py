...
@pytest.fixture
def doubao_instance():
    """Create a DoubaoTool instance with test configuration."""
    return DoubaoTool(
...
def test_init(doubao_instance):
    """Test that DoubaoTool initializes with correct attributes."""
    assert doubao_instance.api_key == "test_api_key"
    assert doubao_instance.model == "doubao-seed-1-6-250615"
    assert doubao_instance.system_message == "dummy system message"
    assert doubao_instance.base_url == "https://ark.cn-beijing.volces.com/api/v3"


@pytest.mark.asyncio
async def test_invoke_simple_response(monkeypatch, doubao_instance, invoke_context):
    """Test simple text response from Doubao API."""
    import grafi.tools.llms.impl.doubao_tool
    ...
    monkeypatch.setattr(
        grafi.tools.llms.impl.doubao_tool, "AsyncClient", mock_async_client_cls
    )

    input_data = [Message(role="user", content="Say hello")]
    result_messages = []
    async for message_batch in doubao_instance.invoke(invoke_context, input_data):
    ...
    mock_async_client_cls.assert_called_once_with(
        api_key="test_api_key", base_url="https://ark.cn-beijing.volces.com/api/v3"
    )


@pytest.mark.asyncio
async def test_invoke_function_call(monkeypatch, doubao_instance, invoke_context):
    """Test function call response from Doubao API."""
    import grafi.tools.llms.impl.doubao_tool
    ...
    doubao_instance.add_function_specs(tools)
    result_messages = []
    async for message_batch in doubao_instance.invoke(invoke_context, input_data):
        result_messages.extend(message_batch)
...
@pytest.mark.asyncio
async def test_invoke_api_error(doubao_instance, invoke_context):
    """Test that API errors are properly handled and converted to LLMToolException."""
    from grafi.common.exceptions import LLMToolException

    with pytest.raises(LLMToolException, match="Error code|Doubao API"):
        async for _ in doubao_instance.invoke(
            invoke_context, [Message(role="user", content="Hello")]
        ):
            pass


def test_to_dict(doubao_instance):
    """Test conversion of DoubaoTool instance to dictionary format."""
    result = doubao_instance.to_dict()
    assert result["name"] == "DoubaoTool"
    assert result["type"] == "DoubaoTool"
...
def test_prepare_api_input(doubao_instance):
    """Test preparation of input data for Doubao API format."""
    input_data = [
    ...
    doubao_instance.add_function_specs(
        [
            FunctionSpec(
                name="get_weather",
                description="Get weather",
    ...
    api_messages, api_functions = doubao_instance.prepare_api_input(input_data)
