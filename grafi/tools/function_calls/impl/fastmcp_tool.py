...
class FastMCPTool(FunctionCallTool):
    """
    FastMCPTool extends FunctionCallTool to provide web search functionality using the MCP API.
    """

    # Set up API key and MCP client
    name: str = "FastMCPTool"
    type: str = "FastMCPTool"
    ...
    @classmethod
    def builder(cls) -> "FastMCPToolBuilder":
        """
        Return a builder for FastMCPTool.
        """
        return FastMCPToolBuilder(cls)
    ...
    async def _a_get_function_specs(self) -> None:
        ...
        logger.debug(f"Initializing FastMCPTool with config: {client_config}")
    ...
    def invoke(self, invoke_context: InvokeContext, input_data: Messages) -> Messages:
        raise NotImplementedError(
            "FastMCPTool does not support synchronous invoke. Use a_invoke instead."
        )
    ...
    async def a_invoke(
        ...
        """
        Invoke the FastMCPTool with the provided input data.
        Args:
...
class FastMCPToolBuilder(FunctionCallToolBuilder[FastMCPTool]):
    """
    Builder for FastMCPTool.
    """

    def client_config(self, config: Dict[str, Any]) -> "FastMCPToolBuilder":
        """
        Set the client configuration for the FastMCPTool.
        """
    ...
    def build(self) -> None:
        raise NotImplementedError(
            "FastMCPTool does not support synchronous invoke. Use a_build instead."
        )

    async def a_build(self) -> "FastMCPTool":
