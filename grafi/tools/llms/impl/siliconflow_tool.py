...
class SiliconFlowTool(LLM):
    """
    A class representing the SiliconFlow language model implementation.

    This class provides methods to interact with SiliconFlow's API for natural language processing tasks.

    Attributes:
        api_key (str): The API key for authenticating with SiliconFlow (SILICONFLOW_API_KEY).
        model (str): The name of the SiliconFlow model to use (default is 'Qwen/QwQ-32B').
        base_url (str): The base URL for SiliconFlow API endpoint.
    """
    ...
    @classmethod
    def builder(cls) -> "SiliconFlowToolBuilder":
        """
        Return a builder for SiliconFlowTool.

        This method allows for the construction of a SiliconFlowTool instance with specified parameters.
        """
        return SiliconFlowToolBuilder(cls)
    ...
    def prepare_api_input(
        ...
        """
        Prepare the input data for the SiliconFlow API.
    ...
    async def invoke(
        ...
        """
        Invoke the SiliconFlow API to generate responses.
        ...
        try:
            ...
            raise LLMToolException(
                tool_name=self.name,
                model=self.model,
                message=f"SiliconFlow API streaming failed: {exc}",
            ...
            raise LLMToolException(
                tool_name=self.name,
                model=self.model,
                message=f"Unexpected error during SiliconFlow streaming: {e}",
                invoke_context=invoke_context,
                cause=e,
            ) from e
    ...
    def to_stream_messages(self, chunk: ChatCompletionChunk) -> Messages:
        """
        Convert a SiliconFlow API streaming chunk to a Message object.

        This method extracts relevant information from the streaming chunk and constructs a Message object.

        Args:
            chunk (ChatCompletionChunk): The streaming chunk from the SiliconFlow API.
    ...
    def to_messages(self, response: ChatCompletion) -> Messages:
        """
        Convert a SiliconFlow API response to a Message object.

        This method extracts relevant information from the API response and constructs a Message object.

        Args:
            response (ChatCompletion): The response object from the SiliconFlow API.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Convert the SiliconFlowTool instance to a dictionary.

        Returns:
            dict: A dictionary containing the attributes of the SiliconFlowTool instance.
        """
        return {
            **super().to_dict(),
        }
...
class SiliconFlowToolBuilder(LLMBuilder[SiliconFlowTool]):
    """
    Builder class for SiliconFlowTool.
    
    Provides a fluent interface for constructing SiliconFlowTool instances.
    """

    def api_key(self, api_key: Optional[str]) -> Self:
        """
        Set the API key for SiliconFlow authentication.
    ...
    def base_url(self, base_url: str) -> Self:
        """
        Set the base URL for SiliconFlow API endpoint.
        
        Args:
            base_url (str): The base URL to use (e.g., 'https://api.siliconflow.cn/v1').
